...
@section('title', 'Tambah Konfigurasi Gemini API')
    ...
    <div class="row">
        <div class="col-md-8 offset-md-2">
            <div class="card">
                ...
                <form action="{{ route('admin.gemini-configs.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('api_key') is-invalid @enderror" 
                                      ...
                                      placeholder="Masukkan API key dari Google Cloud Console" required>{{ old('api_key') }}</textarea>
                            @error('api_key')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('model') is-invalid @enderror" id="model" name="model" required>
                                ...
                                <option value="gemini-2.5-flash" {{ old('model') == 'gemini-2.5-flash' ? 'selected' : '' }}>Gemini 2.5 Flash (Latest)</option>
                                <option value="gemini-2.0-flash" {{ old('model') == 'gemini-2.0-flash' ? 'selected' : '' }}>Gemini 2.0 Flash (Recommended)</option>
                                <option value="gemini-1.5-pro" {{ old('model') == 'gemini-1.5-pro' ? 'selected' : '' }}>Gemini 1.5 Pro</option>
                                <option value="gemini-1.5-flash" {{ old('model') == 'gemini-1.5-flash' ? 'selected' : '' }}>Gemini 1.5 Flash</option>
                            </select>
                            @error('model')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.1" min="0" max="2" 
                                           ...
                                           id="temperature" name="temperature" value="{{ old('temperature', 0.7) }}" required>
                                    @error('temperature')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" min="1" max="4096" 
                                           ...
                                           id="max_output_tokens" name="max_output_tokens" value="{{ old('max_output_tokens', 1024) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('system_prompt') is-invalid @enderror" 
                                      id="system_prompt" name="system_prompt" rows="5" required>{{ old('system_prompt', 'Anda adalah chatbot asisten administrasi RT (Rukun Tetangga) yang helpful dan ramah.
...
Berikan jawaban yang jelas, ringkas, dan helpful dalam Bahasa Indonesia.') }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      placeholder="Deskripsi konfigurasi ini...">{{ old('description') }}</textarea>
                            @error('description')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
